...
BlockCipher::BlockCipher()
  ...
  memset(salt_.get(), 0, kSaltLen);
  memset(iv_.get(), 0, EVP_MAX_IV_LENGTH);
  memset(key_.get(), 0, EVP_MAX_KEY_LENGTH);
}
...
bool BlockCipher::Encrypt(const std::string& input,
  ...
  memset(ciphertext.get(), 0, c_len + 1);
...
bool BlockCipher::Decrypt(const std::string& input,
  ...
  memset(plaintext.get(), 0, p_len + AES_BLOCK_SIZE + 1);

  // Parse the input string for iv and ciphertext.
  memset(salt_.get(), 0, 8);
